...
**Один репозиторий — один индикатор.** Этот репозиторий = GDI. Новый индикатор — новый репозиторий, та же изоляция: ядро считает снимок, UI и биржи не входят в формулу. Соединять индикаторы можно будет позже по снимкам (`GravitySnapshot` и аналоги), не смешивая код и не таща чужие веса в G.
...
- Новых индикаторов в этом репозитории (один репо — один индикатор)
